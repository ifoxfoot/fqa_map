...
FQA_states <- states() %>% 
  ...
  mutate(Notes = case_when(FQA_database == "wyoming_wetlands_2017" ~ "Wetlands only",
                           ...
                           FQA_database %in% c("delaware_2013") ~ "Approved with reservations. Use alternate database if posssible")) %>% 
...
```{r}
#read in data
fqa_coastline <- read_sf(here("regional_fqa_simple.gpkg"))
...
#native only
native_only <- fqa_grouped %>% 
  filter(total_species == total_native_species)
...
tm_shape(fqa_bind) +
  tm_polygons("FQA_database", legend.show = F, popup.vars = c("Notes", "total_species", "total_native_species")) +
