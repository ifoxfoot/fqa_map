...
coastal_plain_southeast <- us_ecor_state %>% 
  ...
  st_union() %>% 
  st_sf() %>% 
  mutate(FQA_database = "coastal_plain_2013")
...
piedmont_region <- us_ecor_state %>% 
  ...
  st_union() %>% 
  st_sf() %>% 
  mutate(FQA_database = "southeastern_piedmont_2013")
```
...
tm_polygons("FQA_database", legend.show = FALSE) +
...
#save
st_write(fqa_bind_notes, "shapefiles_written/regional_fqa_simple.shp", append = F)
```
